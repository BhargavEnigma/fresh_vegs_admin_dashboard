...
export function LoginPage() {
  ...
  async function onSend(values) {
    try {
      ...
      setStep("verify");
      toast.push({ variant: "success", title: "OTP sent", description: "Check your phone for OTP." });
    } catch (e) {
      const msg = e?.response?.data?.error?.message || e?.message || "Failed to send OTP";
      toast.push({ variant: "error", title: "Send OTP failed", description: msg });
    }
  }
  ...
  async function onVerify(values) {
    try {
      ...
      login(resp);
      toast.push({ variant: "success", title: "Logged in", description: "Welcome back." });
      navigate("/", { replace: true });
    } catch (e) {
      const msg = e?.response?.data?.error?.message || e?.message || "OTP verification failed";
      toast.push({ variant: "error", title: "Verify failed", description: msg });
    }
  }
            ...
            {step === "send" ? (
              <form onSubmit={sendForm.handleSubmit(onSend)} className="space-y-4">
                <div className="space-y-2">
                  <Label>Phone</Label>
                  <Input placeholder="91XXXXXXXXXX or XXXXXXXXXX" {...sendForm.register("phone")} />
                  {sendForm.formState.errors.phone ? (
                    <p className="text-xs text-red-600">{sendForm.formState.errors.phone.message}</p>
                  ) : null}
                </div>
              ...
              <form onSubmit={verifyForm.handleSubmit(onVerify)} className="space-y-4">
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3 text-sm dark:border-slate-800 dark:bg-slate-900/30">
                  OTP request: <span className="font-mono text-xs">{otpRequestId}</span>
                  <div className="mt-1 text-xs text-slate-500">Phone: {phone}</div>
                </div>
                ...
                <div className="space-y-2">
                  <Label>OTP</Label>
                  <Input placeholder="Enter OTP" {...verifyForm.register("otp")} />
                  {verifyForm.formState.errors.otp ? (
                    <p className="text-xs text-red-600">{verifyForm.formState.errors.otp.message}</p>
                  ) : null}
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  <Button type="submit" className="w-full" disabled={verifyForm.formState.isSubmitting}>
                    {verifyForm.formState.isSubmitting ? "Verifying…" : "Verify & Login"}
                  </Button>
                </div>
              </form>
            )}
