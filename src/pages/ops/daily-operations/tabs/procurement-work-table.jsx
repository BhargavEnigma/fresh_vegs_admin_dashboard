...
function quantity(item, value, field) {
  if (field && item.product_group_rows?.length) {
    const portions = item.product_group_rows
      .map((row) => {
...
export function ProcurementWorkTable({
  ...
  const viewItems = useMemo(
    () => {
      ...
      return view === PROCUREMENT_VIEWS.HISTORY
        ? rows
  ...
  const items = useMemo(
    () => viewItems.filter((item) => {
      ...
      return !term || [item.product_name, item.product?.name, item.procurement_unit, item.vendor_name]
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        {!isHistory && (isAdmin || isWarehouseManager) && !isClosed ? <Button variant="outline" onClick={onAutoAssign} className="rounded-xl" disabled={isUpdating || autoAssignDisabled}><ShoppingCart className="mr-2 h-4 w-4" />Auto Assign</Button> : null}
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1140px] text-left text-xs">
              <thead className="sticky top-0 z-10 bg-slate-50 text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:bg-slate-900"><tr>
                ...
                {isHistory ? <><th className="px-4 py-4 text-right">Completed Qty</th><th className="px-4 py-4">Vendor</th><th className="px-4 py-4 text-right">Final Cost</th></> : <><th className="px-4 py-4 text-right">Total Needed</th><th className="px-4 py-4 text-right">Vendor Arranged</th><th className="px-4 py-4 text-right">Received</th><th className="px-4 py-4 text-right">Still to Assign</th><th className="w-[220px] px-4 py-4 text-center">Assigned Vendor</th></>}
                <th className="px-4 py-4">Current Step</th><th className="px-5 py-4 text-right">Action</th>
              </tr></thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">{items.map((item) => {
                ...
                return <tr key={item.id || item.procurement_cost_id} className={cn("bg-white dark:bg-slate-950", isHistory && "bg-emerald-50/20 dark:bg-emerald-950/5")}>
                  <td className="px-5 py-4"><p className="font-bold text-slate-950 dark:text-white">{item.product_name || item.product?.name || "Product"}</p><p className="mt-1 text-[10px] font-semibold uppercase text-slate-400">{item.pack_label || item.pack?.pack_label || item.procurement_unit || "unit"}</p></td>
                  {isHistory ? <>
                    <td className="px-4 py-4 text-right font-extrabold text-emerald-700 dark:text-emerald-300">{quantity(item, item.received_quantity)}</td>
                    <td className="px-4 py-4">{item.vendor_name || "—"}</td>
                    <td className="px-4 py-4 text-right font-bold">{formatPaiseToRupees(item.total_cost_paise)}</td>
                  </> : <>
                  ...
                  <td className="px-5 py-4 text-right">{!isHistory && canStartVendorAssignment(item, view) && (isAdmin || isWarehouseManager) && !isClosed ? <Button size="sm" onClick={() => onAssignVendor(item)} disabled={isUpdating}>Assign Vendor</Button> : !isHistory && ["warehouse_receipt", "receive_remaining"].includes(code) && !isClosed ? <Button size="sm" onClick={() => onReceive(item)} disabled={isUpdating}>Receive Stock</Button> : !isHistory && code === "resolve_issue" ? <Button size="sm" variant="outline" onClick={() => onCheckProblem(item)}>Check Problem</Button> : onViewDetails ? <Button size="sm" variant="ghost" onClick={() => onViewDetails(item)}>View Details</Button> : <span className="text-[11px] font-medium text-slate-400">No action needed</span>}</td>
                </tr>;
              })}</tbody>
            </table>
          </div>
        ...
        <DialogContent className="max-h-[85vh] max-w-3xl overflow-hidden rounded-2xl p-0">
          ...
          <div className="max-h-[calc(85vh-90px)] space-y-3 overflow-y-auto p-5 thin-scrollbar">
            {(vendorDetails?.assignments || []).map((assignment) => {
              ...
              return (
                <div key={assignment.id || `${assignment.vendor_user_id}-${assignment.allocated_quantity}`} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0"><p className="truncate text-base font-extrabold text-slate-950 dark:text-white">{vendorName(assignment)}</p><p className="mt-1 text-[11px] font-medium text-slate-500">Vendor allocation #{assignment.id || "—"}</p></div>
                    <Badge variant={status.variant} className="shrink-0 rounded-full">{status.label}</Badge>
                  </div>
                  <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                    <div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-900"><p className="text-[9px] font-bold uppercase tracking-wider text-slate-400">Allocated</p><p className="mt-1 text-sm font-extrabold">{quantity(assignmentItem, assignment.allocated_quantity)}</p></div>
                    <div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-900"><p className="text-[9px] font-bold uppercase tracking-wider text-slate-400">Supplied</p><p className="mt-1 text-sm font-extrabold">{quantity(assignmentItem, assignment.supplied_quantity)}</p></div>
                  ...
                  {Number(assignment.allocation_revision || 1) > 1 ? (
                    <div className="mt-3 grid grid-cols-3 gap-2 rounded-xl border border-blue-200 bg-blue-50 p-3 text-xs dark:border-blue-900 dark:bg-blue-950/30">
                      <div><p className="text-[9px] font-bold uppercase text-slate-400">Previous</p><strong>{quantity(assignmentItem, assignment.previous_allocated_quantity)}</strong></div>
                      <div><p className="text-[9px] font-bold uppercase text-slate-400">Added</p><strong className="text-blue-700 dark:text-blue-300">+{quantity(assignmentItem, assignment.last_allocation_delta)}</strong></div>
                      <div><p className="text-[9px] font-bold uppercase text-slate-400">New Total</p><strong>{quantity(assignmentItem, assignment.allocated_quantity)}</strong></div>
                    </div>
                  ) : null}
