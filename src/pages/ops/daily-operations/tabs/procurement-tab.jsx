...
const itemMode = (item) => item.procurement_mode === "bulk" ? "bulk" : "pack";
const itemUnit = (item) => {
  const unit = String(
...
export function ProcurementTab({
  ...
  const vendorCataloguesQuery = useQuery({
    queryKey: ["admin", "vendorCatalogues"],
    queryFn: async () => {
      const vendors = (await VendorService.list()).filter(
    ...
    enabled: Boolean(assigningItem),
    staleTime: 5 * 60 * 1000,
  });


  const assignMutation = useMutation({
    mutationFn: ({ procurementCostId, assignments }) =>
  ...
  const assignmentsByCost = useMemo(() => {
    return (assignmentsQuery.data || []).reduce((map, assignment) => {
      const key = String(assignment.procurement_cost_id);
      if (!map[key]) map[key] = [];
  ...
  const openVendorCheckIn = (item) => {
    const assignment = getItemAssignments(item).find((entry) =>
      ["confirmed", "dispatched"].includes(entry.status)
    );
    const params = new URLSearchParams();
    if (operation?.delivery_date) params.set("date", operation.delivery_date);
    const vendorUserId = assignment?.vendor_user_id || assignment?.vendor?.id;
    if (vendorUserId) params.set("vendor_user_id", vendorUserId);
    navigate(`/ops/vendor-check-in?${params.toString()}`);
  };
  ...
  const submitAssignments = () => {
    ...
    const totalCoverageScaled = completedTotal + allocated;
    if (totalCoverageScaled > parseQuantityScaled(assigningItem?.required_quantity || "0")) {
      toast.warning(`Total coverage (completed + new) cannot exceed ${formatQuantityWithUnit(assigningItem?.required_quantity, itemUnit(assigningItem))}`);
      return;
    }
    ...
    <div className="space-y-4">
                          ...
                          <div className="flex gap-1.5">
                            {vendorManaged ? (
                              <Button
                                ...
                                title={actionFlow.canOpenVendorCheckIn ? "Receive dispatched vendor delivery" : "Vendor must confirm or dispatch first"}
                              >
            ...
            <tbody>
              {filteredItems.map((item) => {
                ...
                return (
                  <tr
                    key={item.id}
                    className={`align-middle transition-colors [&>td]:border-b [&>td]:border-slate-100 dark:[&>td]:border-slate-900 ${
                      isDirty ? "bg-amber-50/40 dark:bg-amber-950/20" : "hover:bg-slate-50/70 dark:hover:bg-slate-900/30"
                    }`}
                  >
                    ...
                    <td className={`sticky right-0 w-[270px] min-w-[270px] border-l border-slate-100 px-4 py-4 text-right shadow-[-10px_0_20px_-20px_rgba(15,23,42,0.35)] dark:border-slate-800 ${
                      isDirty ? "bg-amber-50 dark:bg-amber-950" : "bg-white dark:bg-slate-950"
                    }`}>
                      {!isClosed && isAdmin && (
            ...
            <div className="rounded-xl border bg-slate-50 p-3 dark:bg-slate-900">
              ...
              <p className="text-sm text-slate-500">
                {itemMode(assigningItem) === "bulk"
                  ...
                  : (assigningItem.pack_label || assigningItem.pack?.pack_label || "Pack")} · Required quantity:{" "}
                <strong>{formatQuantityWithUnit(assigningItem.required_quantity, itemUnit(assigningItem))}</strong>
              </p>
            </div>
              ...
              <div className="rounded-xl border border-blue-100 bg-blue-50/50 p-3.5 dark:border-blue-900/40 dark:bg-blue-950/20">
                ...
                <div className="space-y-1.5">
                    ...
                    <div key={assignment.id} className="flex justify-between items-center text-xs text-slate-700 dark:text-slate-350">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <strong className="text-slate-900 dark:text-white">
                          {formatQuantityWithUnit(assignment.allocated_quantity, itemUnit(assigningItem))}
                        </strong>
                      </div>
                    </div>
                  ))}
                  <div className="border-t border-blue-200/50 dark:border-blue-900/30 pt-1.5 mt-1.5 flex justify-between items-center text-xs font-bold text-blue-900 dark:text-blue-300">
                    ...
                    <span>{formatQuantityWithUnit(completedTotal, itemUnit(assigningItem))}</span>
                  </div>
                </div>
              </div>
            )}
            <div className="space-y-3">
              {allocationRows.map((row, index) => {
                ...
                const options = matches.map((match) => ({
                  ...
                  label: `${match.vendor.company_name} — min ${formatQuantityWithUnit(match.minimumQuantity, itemUnit(assigningItem))}, max ${
                    match.maximumQuantity === null ? "unlimited" : formatQuantityWithUnit(match.maximumQuantity, itemUnit(assigningItem))
              ...
              <div>
                ...
                <div className="mt-2 space-y-1 text-xs text-slate-500">
                  <div>
                    ...
                    <strong className="text-slate-800 dark:text-slate-200">
                      {formatQuantityWithUnit(completedTotal, itemUnit(assigningItem))}
                    </strong>
                  </div>
                  <div>
                    New allocation:{" "}
                  ...
                  <div className="border-t border-slate-200 dark:border-slate-800 pt-1 mt-1 font-bold text-slate-900 dark:text-white flex items-center gap-1">
                    ...
                    <span>/ {formatQuantityWithUnit(assigningItem.required_quantity, itemUnit(assigningItem))}</span>
                  </div>
                </div>
              </div>
      ...
      {autoAssignResult && (
        <Dialog open onOpenChange={(open) => !open && setAutoAssignResult(null)}>
          <DialogContent className="overflow-hidden border-0 p-0 max-w-3xl">
            <div className="relative overflow-hidden bg-gradient-to-br from-indigo-950 via-slate-950 to-emerald-950 px-6 py-6 text-white">
                ...
                <div>
                  ...
                  <div className="overflow-x-auto rounded-2xl border border-slate-200 dark:border-slate-800">
                    <table className="w-full min-w-[650px] text-left text-sm">
                      <thead className="bg-slate-50 text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:bg-slate-900">
                        <tr>
